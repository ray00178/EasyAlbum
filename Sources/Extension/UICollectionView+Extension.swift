...
extension UICollectionView {
    ...
    func dequeueCell<T: UICollectionViewCell>(_ t: T.Type, indexPath: IndexPath) -> T {
        let identifier = String(describing: t)
        guard let cell = self.dequeueReusableCell(withReuseIdentifier: identifier, for: indexPath) as? T
    ...
    func dequeueHeader<T: UICollectionReusableView>(_ t: T.Type, indexPath: IndexPath) -> T {
        let identifier = String(describing: t)
        guard let header = self.dequeueReusableSupplementaryView(ofKind: UICollectionView.elementKindSectionHeader, withReuseIdentifier: identifier, for: indexPath) as? T
        else { fatalError("Can not found \(t.description()) type!") }
    ...
    func dequeueFooter<T: UICollectionReusableView>(_ t: T.Type, indexPath: IndexPath) -> T {
        let identifier = String(describing: t)
        guard let footer = self.dequeueReusableSupplementaryView(ofKind: UICollectionView.elementKindSectionFooter, withReuseIdentifier: identifier, for: indexPath) as? T
        else { fatalError("Can not found \(t.description()) type!") }
